...
import { UvfVaultKeys, VaultMetadata } from '../../src/common/crypto';
...
describe('crypto', () => {
  ...
  describe('UvfVaultKeys', () => {
    it('create()', async () => {
      const orig = await UvfVaultKeys.create();

      expect(orig).to.be.not.null;
    });

    // TODO https://github.com/encryption-alliance/unified-vault-format/pull/19 fails - what to do with it?
//     it('recover() succeeds for valid key', async () => {
//       let recoveryKey = `
//         pathway lift abuse plenty export texture gentleman landscape beyond ceiling around leaf cafe charity
//         border breakdown victory surely computer cat linger restrict infer crowd live computer true written amazed
...
//     });
    ...
    it('recover() fails for invalid recovery key', async () => {
      const noMultipleOfTwo = UvfVaultKeys.recover('pathway');
      const notInDict = UvfVaultKeys.recover('hallo bonjour');
      const wrongLength = UvfVaultKeys.recover('pathway lift');
      const invalidCrc = UvfVaultKeys.recover(`
        pathway lift abuse plenty export texture gentleman landscape beyond ceiling around leaf cafe charity
        border breakdown victory surely computer cat linger restrict infer crowd live computer true written amazed
        investor boot depth left theory snow whereby terminal weekly reject happiness circuit partial cup wrong
        `);
    ...
    describe('Prove Vault Ownership using Vault Admin Password', () => {
      ...
      it('decryptWithAdminPassword() with wrong pw', () => {
        return expect(UvfVaultKeys.decryptWithAdminPassword('wrong', wrapped.wrappedMasterkey, wrapped.wrappedOwnerPrivateKey, wrapped.ownerPublicKey, wrapped.salt, wrapped.iterations)).to.eventually.be.rejectedWith(UnwrapKeyError);
      });
      // TODO https://github.com/encryption-alliance/unified-vault-format/pull/19 fails - what to do with it?
//       it('decryptWithAdminPassword() with correct pw', () => {
//         return expect(UvfVaultKeys.decryptWithAdminPassword('pass', wrapped.wrappedMasterkey, wrapped.wrappedOwnerPrivateKey, wrapped.ownerPublicKey, wrapped.salt, wrapped.iterations)).to.eventually.be.fulfilled;
//       });
    });

    describe('After creating new key material', () => {
      let vaultKeys: UvfVaultKeys;

      beforeEach(async () => {
        vaultKeys = await TestUvfVaultKeys.create();
      });
...
//         });
//         it('recover() imports original key', async () => {
//           const recovered = await UvfVaultKeys.recover(recoveryKey);
//
//           const oldMasterKey = await crypto.subtle.exportKey('jwk', vaultKeys.masterKey);
//           const newMasterKey = await crypto.subtle.exportKey('jwk', recovered.masterKey);
//           expect(newMasterKey).to.deep.include({
...
//       });
  ...
  describe('VaultMetadata', () => {
    ...
    it('encryptWithMasterKey() and decryptWithMasterKey()', async () => {
      const vaultKeys = await UvfVaultKeys.create();
...
class TestUvfVaultKeys extends UvfVaultKeys {
  constructor(key: CryptoKey) {
    super(key);
  }
  ...
  static async create() {
    ...
    return new TestUvfVaultKeys(key);
  }
}
