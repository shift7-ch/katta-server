...
import { VaultMetadataJWEStorageDto, VaultMetadataJWEAutomaticAccessGrantDto } from './backend';
...
export class VaultKeys {
  ...
  // in Cipherduck, the vault masterKey is used to encrypt the vault metadata JWE using A256KW
  ...
  public static async decryptWithUserKey(jwe: string, userPrivateKey: CryptoKey): Promise<VaultKeys> {
    ...
    try {
      ...
      const masterkey = crypto.subtle.importKey('raw', rawKey, VaultKeys.MASTERKEY_KEY_DESIGNATION, true,
      // / start cipherduck modification
      ['wrapKey', 'unwrapKey']
      // \ end cipherduck modification
      );
...
export class VaultMetadata {
  ...
  readonly storage: VaultMetadataJWEStorageDto;
  ...
  protected constructor(storage: VaultMetadataJWEStorageDto, automaticAccessGrant: VaultMetadataJWEAutomaticAccessGrantDto, keys: Record<string,CryptoKey>, latestFileKey: string, nameKey: string) {
   ...
   */
  public static async create(storage: VaultMetadataJWEStorageDto, automaticAccessGrant: VaultMetadataJWEAutomaticAccessGrantDto): Promise<VaultMetadata> {
    ...
    return new VaultMetadata(storage, automaticAccessGrant, keys, fileKeyId, nameKeyId);
  }
  ...
  public static async decryptWithMasterKey(jwe: string, masterKey: CryptoKey): Promise<VaultMetadata> {
      ...
      return new VaultMetadata(
        payload['com.cipherduck.storage'],
  ...
  const keysExported: Record<string,string> = {};
    ...
    const payload = {
        ...
        // TODO https://github.com/shift7-ch/cipherduck-hub/issues/19 finalize kdf (https://github.com/encryption-alliance/unified-vault-format/pull/21)
        kdf: "1STEP-HMAC-SHA512",
        'com.cipherduck.storage': this.storage,
  ...
  public async hashDirectoryId(cleartextDirectoryId: string): Promise<string> {
    ...
    // TODO https://github.com/shift7-ch/cipherduck-hub/issues/19 use rawFileKey,rawNameKey for rootDirHash for now - should depend on nameKey only!!
