    ...
    public void makeS3Bucket(
        ...
        try (final S3Client s3 = s3Builder.build()) {
            try {
                ...
                throw new ClientErrorException(String.format("Bucket %s already exists", bucketName), Response.Status.CONFLICT);
            } catch (final NoSuchBucketException e) {
                // Ignore, as expected
            } catch (final ClientErrorException e) {
                throw e;
            } catch (final Exception e) {
                // Any other exception doesn't necessarily mean we can't create the bucket
                log.warn(String.format("Failure heading bucket %s but try to create it", bucketName), e);
            }
            ...
            try {
                ...
                throw new ClientErrorException(String.format("Failure creating bucket %s and uploading vault template", bucketName), Response.Status.BAD_REQUEST, e);
            }
        }
    }
}
